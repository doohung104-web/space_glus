...
def load_revos_json(base_image_dir, is_train=True, set_name=None):
        
    base_image_dir = os.path.join(base_image_dir, 'ReVOS')
    data_split = "train" if is_train else "valid_u"
    if set_name != None:
        data_split = set_name
        
    image_root=os.path.join(base_image_dir, 'JPEGImages')
    json_file=os.path.join(base_image_dir, f'meta_expressions_{data_split}_.json')
    ...
    if data_split == "train":
        ...
        for vid in videos:
            ...
            for exp_id, exp_dict in vid_data['expressions'].items():
                meta = {}
                meta['str_id'] = f'revos_{vid}_{exp_id}'
                meta['video'] = vid
