...
def parse_args(args):
    parser = argparse.ArgumentParser(description="KFS Model Training")
    ...
    parser.add_argument("--rvos_seg_data", default="mevis_train||refytvos_train", type=str)
    parser.add_argument('--rvos_sample_ratio', default="4000||15000", type=str,)
    ...
    parser.add_argument('--rvos_max_image_token', type=int, default=12) 
...
def main(args):
    ...
    num_added_tokens = tokenizer.add_tokens("[SCORE]")
    args.score_token_idx = tokenizer("[SCORE]", add_special_tokens=False).input_ids[0]
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if True:
            ...
            if epoch % 5 == 4:
                save_dir = os.path.join(args.log_dir, f"ckpt_model_epoch{epoch}")
                torch.distributed.barrier()
                model_engine.save_checkpoint(save_dir)
...
def train(
    ...
    iou_losses = AverageMeter("IoULoss", ":.4f")
    ...
    for global_step in range(args.steps_per_epoch):
        for i in range(args.grad_accumulation_steps):
            ...
            iou_losses.update(iou_loss.item(), input_dict["input_ids"].size(0))
        ...
        if global_step % args.print_freq == 0:
            ...
            if args.local_rank == 0:
                ...
                writer.add_scalar("train/iou_loss", iou_losses.avg, global_step)
                writer.add_scalar(
...
def rvos_validate(val_loader, model_engine, epoch, writer, args):
    ...
    for input_dict in tqdm.tqdm(val_loader):
        ...
        #convert to float
        pred_ious = pred_ious.cpu().numpy().astype(np.float32)
        #save score to a tjson file
        for i, output_path_i in enumerate(input_dict['output_paths']):
            assert output_path_i.endswith('.tjson'), f'output_path_i: {output_path_i} must end with .tjson'
            ...
            with open(output_path_i, 'w') as f:
                f.write(f'{{"iou": {pred_ious[i].item()}}}')
