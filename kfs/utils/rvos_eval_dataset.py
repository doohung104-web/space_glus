...
class RVOSEvalDataset(torch.utils.data.Dataset):
    ...
    def load_data(self, ):
        ...
        for idx_vd, vid_dict in tqdm(enumerate(metas), desc=f'Loading {self.rvos_dataset_name} ...'):
            ...
            for file_path in record["file_names"]:
                ...
                output_path = osp.join(self.output_dir, "Annotations", video_name, exp_id, file_name + '.json')
    ...
    def __getitem__(self, idx):
        ...
        if self.max_image_token != 1:
            ...
            #do not use local sampling, we use global sampling to make sure the same frame is selected
            # idx_select = list(range(0, len(data['video_frame_path_list']), len(data['video_frame_path_list']) // self.max_image_token))
        else:
            idx_select = [idx_seg, ]
