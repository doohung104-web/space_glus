...
def eval_queue(q, rank, out_dict, mask_path, pred_path):
    while not q.empty():
        vid_key2, vid_key, exp_id = q.get()
        exp_name = "{}_{}".format(vid_key, exp_id)

        mask_path_vid = os.path.join(mask_path, vid_key)
        pred_path_vid_exp = os.path.join(pred_path, vid_key2, exp_id)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--meta_exp_path", type=str, help=" path to ReasonVOS/meta_expressions.json")
    parser.add_argument("--mask_path", type=str, help="path to ReasonVOS/Annotations")
    parser.add_argument("--pred_path", type=str, help="path to predicted masks")
    parser.add_argument("--save_name", type=str, default="./reasonvos")
    ...
    for vid_name in meta_exp.keys():
        ...
        for sample in vid['expressions']:
            ...
            vid_key2 = vid_name
            queue.put([vid_key2, vid_key, str(exp_id)])
